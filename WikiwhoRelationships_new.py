...
class Wikiwho():
    ...
    def create_wp_session(language=None):
        ...
        user = "FaFlo@wikiwho2"
        password = "f0fa3ncao4q9pi1gqef79sliuvja8rhj"
        headers = {'User-Agent': 'research project script', 'From': 'fabian.floeck@gesis.org'}
                        ...
                        if (elem != revision_curr.wikipedia_id) and (elem in self.revisions.keys()):
                            if (self.revisions[elem].contributor_name != revision_curr.contributor_name):
                                if (elem in relation.revert.keys()):
                                    relation.revert.update({elem: relation.revert[elem] + [word_prev.value]})
                                else:
                                    relation.revert.update({elem: [word_prev.value]})
                            else:
                                if (elem in relation.self_revert.keys()):
                                    relation.self_revert.update({elem: relation.self_revert[elem] + [word_prev.value]})
                                else:
                                    relation.self_revert.update({elem: [word_prev.value]})
                                        ...
                                        if (elem in self.revisions.keys()):
                                            if (self.revisions[word_prev.revision].contributor_name != revision_curr.contributor_name):
                                                ...
                                                if (word_prev.revision in relation.self_reintroduced.keys()):
                                                    relation.self_reintroduced.update({word_prev.revision : relation.self_reintroduced[word_prev.revision] + [word_prev.value]})
                                                else:
                                                    relation.self_reintroduced.update({word_prev.revision : [word_prev.value]})
    ...
    def printRelationships(self, relations, order):
        ...
        header = ["revision", "author", "deleted(-)", "revert(-)", "reintroduced(+)", "redeleted(+)", "oadds", "total", "self-deleted", "self-revert", "self-reintroduced", "self-redeleted"]
        print "\t".join(header)
