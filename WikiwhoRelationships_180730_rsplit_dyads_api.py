...
class Wikiwho():
    ...
    def __init__(self):
        ...
        self.CHANGE_PERCENTAGE = -0.40
        self.PREVIOUS_LENGTH = 1000
        self.CURR_LENGTH = 1000
        self.FLAG = "move"
        self.UNMATCHED_PARAGRAPH = 0.0
        self.WORD_DENSITY = 10
        self.WORD_LEN = 100

        # Word global id.
        self.WORD_ID = 0

    def get_page_data_from_wp_api_extended(self, params):
        """
        Example params:
    ...
    def analyseArticle(self, article_name):
        ...
        api_params = {'action': 'query',
                       'titles': article_name,
                       'prop': 'revisions',
                       'rvprop': 'content|ids|timestamp|comment|flags|user|userid|size|sha1',
                       'rvlimit': 'max',
                       'continue': '',
                       'format': 'json',
                       'rvdir': 'newer'}
        ...
        i = 0
        for rev_data in self.get_page_data_from_wp_api_extended(api_params):
            vandalism = False
            ...
            if rev_data['sha1'] in self.spam:
                vandalism = True

            # TODO: SPAM detection: DELETION
            if 'comment' in rev_data.keys() and rev_data['comment'].find(self.FLAG) > 0:
                pass
            else:
                if (revision_prev.length > self.PREVIOUS_LENGTH) and (len(rev_data['*']) < self.CURR_LENGTH) and (((len(
                        rev_data['*']) - revision_prev.length) / float(revision_prev.length)) <= self.CHANGE_PERCENTAGE):
                    vandalism = True
            ...
            if not vandalism:
                ...
                relation = Relation()
                relation.revision = int(rev_data['revid'])
                relation.length = len(rev_data['*'])

                # Some revisions don't have contributor.
                if 'userid' in rev_data.keys():
                    revision_curr.contributor_id = rev_data['userid']#revision.getContributor().getId()
                    revision_curr.contributor_name = rev_data['user'].encode('utf-8')
                    relation.author = rev_data['user'].encode('utf-8')
                else:
                    revision_curr.contributor_id = 'Not Available ' + str(rev_data['revid']).encode('utf-8')
                    revision_curr.contribur_name = 'Not Available ' + str(rev_data['revid']).encode('utf-8')
                    relation.author = 'Not Available ' + str(rev_data['revid']).encode('utf-8')
                ...
                # Perform comparison.
                vandalism = self.determineAuthorship(revision_curr, revision_prev, text_curr, relation)
                ...
                if (not vandalism):
                    ...
                    self.revision_order.append((revision_curr.wikipedia_id, False))
                    ...
                    # print
                    self.revision_order.append((revision_curr.wikipedia_id, True))
                    revision_curr = revision_prev
                    self.spam.append(rev_data['sha1'])
                    #API_END
        ...
        #
        #         #TODO: SPAM detection: DELETION
        #         if (revision.getComment()!= None and revision.getComment().find(self.FLAG) > 0):
        #             pass
        #         else:
        #             if (revision_prev.length > self.PREVIOUS_LENGTH) and (len(revision.getText()) < self.CURR_LENGTH) and (((len(revision.getText())-revision_prev.length)/float(revision_prev.length)) <= self.CHANGE_PERCENTAGE):
        #                 vandalism = True
        #                 revision_curr = revision_prev
        #
        #         #if (vandalism):
        ...
        return (self.revisions, self.revision_order, relations)

    def determineAuthorship(self, revision_curr, revision_prev, text_curr, relation):
        ...
        (unmatched_paragraphs_curr, unmatched_paragraphs_prev, matched_paragraphs_prev) = self.analyseParagraphsInRevision(revision_curr, revision_prev, text_curr, relation)

        # Analysis of the sentences in the unmatched paragraphs of the current revision.
        if (len(unmatched_paragraphs_curr)>0):
            (unmatched_sentences_curr, unmatched_sentences_prev, matched_sentences_prev, _) = self.analyseSentencesInParagraphs(unmatched_paragraphs_curr, unmatched_paragraphs_prev, revision_curr, revision_prev, relation)

            #TODO: SPAM detection
            if (len(unmatched_paragraphs_curr)/float(len(revision_curr.ordered_paragraphs)) > self.UNMATCHED_PARAGRAPH):
                possible_vandalism = True

            # Analysis of words in unmatched sentences (diff of both texts).
            if (len(unmatched_sentences_curr)>0):
                (matched_words_prev, vandalism) = self.analyseWordsInSentences(unmatched_sentences_curr, unmatched_sentences_prev, revision_curr, possible_vandalism, relation)
            ...
            for word_prev in unmatched_sentence.words:
                ...
                if not(word_prev.matched):
                    ...
                    #for elem in word_prev.deleted:
                        isredeleted = True
                        if (elem != revision_curr.wikipedia_id) and (elem in self.revisions.keys()):
                            if (self.revisions[elem].contributor_name != revision_curr.contributor_name):
                    ...
                    # for elem in word_prev.freq:
                        ...
                        #    print "Revert in 11", word_prev.value, word_prev.deleted, relation.undo_reintro
                        if (elem != revision_curr.wikipedia_id) and (elem in self.revisions.keys()):
                            if (self.revisions[elem].contributor_name != revision_curr.contributor_name):
                    ...
                    if not isredeleted:
                        if (self.revisions[word_prev.revision].contributor_name != revision_curr.contributor_name):
                            if (word_prev.revision in relation.deleted.keys()):
                                relation.deleted.update({word_prev.revision : relation.deleted[word_prev.revision] + [word_prev.value] })
                            else:
                                relation.deleted.update({word_prev.revision : [word_prev.value] })
                        else:
                            if (word_prev.revision in relation.self_deleted.keys()):
                                relation.self_deleted.update({word_prev.revision : relation.self_deleted[word_prev.revision] + [word_prev.value] })
                            else:
                                relation.self_deleted.update({word_prev.revision : [word_prev.value] })
    ...
    def analyseParagraphsInRevision(self, revision_curr, revision_prev, text_curr, relation):
                        ...
                        for hash_sentence_prev in paragraph_prev.sentences.keys():
                            for sentence_prev in paragraph_prev.sentences[hash_sentence_prev]:
                                ...
                                for word_prev in sentence_prev.words:
                                    ...
                                    if (revision_prev.wikipedia_id not in word_prev.used):
                                        if (word_prev.revision in self.revisions.keys()):
                                            if (self.revisions[word_prev.revision].contributor_name != revision_curr.contributor_name):
                                                #print "HERE",

                                                if len(word_prev.freq) == 0:
                                                    elem = word_prev.revision
                                                else:
                                                    elem = word_prev.freq[-1]
    ...
    def analyseSentencesInParagraphs(self, unmatched_paragraphs_curr, unmatched_paragraphs_prev, revision_curr, revision_prev, relation):
                                        ...
                                        if (word_prev.revision in self.revisions.keys()):
                                            if (self.revisions[word_prev.revision].contributor_name != revision_curr.contributor_name):
                                                ...
                                                if (elem in relation.reintroduced.keys()):
                                                    ...
                                                    relation.reintroduced.update({elem : [word_prev.value] })
                                                #if (word_prev.revision in relation.reintroduced.keys()):
                                                #    relation.reintroduced.update({word_prev.revision : relation.reintroduced[word_prev.revision] + [word_prev.value] })
                                                #else:
                                                #    relation.reintroduced.update({word_prev.revision : [word_prev.value] })
                                            else:
    ...
    def analyseWordsInSentences(self, unmatched_sentences_curr, unmatched_sentences_prev, revision_curr, possible_vandalism, relation):
                ...
                while (pos < len(diff)):
                    ...
                    if (word == word_diff[2:]):
                        ...
                        if (word_diff[0] == ' '):
                            ...
                            word_curr.used.append(revision_curr.wikipedia_id)
                            sentence_curr.words.append(word_curr)
                            relation.added = relation.added + 1 # TODO: we could change this to provide context for oadds
                            self.WORD_ID = self.WORD_ID + 1
    ...
    def printAllRevisions(self, order, revisions):

        for (revision, vandalism) in order:
            if not(vandalism):
                self.printRevision(revisions[revision])
    ...
    def printRevisionTrackAppearance(self, revision):
        ...
        #print text
        #print authors
    def printRelationships(self, relations, order):

        print "Printing relationships"

        header = ["revision", "author", "deleted(-)", "undo_reintro(-)", "undo_delete(-)", "reintroduced(+)", "redeleted(+)", "oadds", "total", "self-deleted", "self_undo_reintro(-)", "self_undo_delete(-)", "self-reintroduced", "self-redeleted"]
        print "\t".join(header)
        ...
        for (revision, vandalism) in order:
            ...
            print str(relation.revision) + "\t" + str(relation.author).decode("utf-8")   + "\t" + str(relation.deleted) + "\t" + str(relation.undo_reintro) + "\t" + str(relation.undo_delete) + "\t" + str(relation.reintroduced)  + "\t" + str(relation.redeleted) + "\t" + str(relation.added)  + "\t"  +  str(relation.total_tokens) + "\t" + str(relation.self_deleted) + "\t" + str(relation.self_undo_reintro) + "\t" + str(relation.self_undo_delete) + "\t" + str(relation.self_reintroduced) + "\t" + str(relation.self_redeleted)

    def printDyads(self, relations, order):

        header = ["s_revision", "s_author", "t_revision", "t_author", "deleted(-,out)", "undo_reintro(-,out)", "undo_delete(-,in)", "reintroduced(+,in)", "redeleted(+,out)", "oadds(=,in)"]
        print "\t".join(header)
        ...
        for (revision, vandalism) in order:
            ...
            targets = set(relation.deleted.keys()) | set(relation.undo_reintro.keys()) | set(relation.undo_delete.keys()) | set(relation.reintroduced.keys()) | set(relation.redeleted.keys())

            #print targets
            for target in targets:
                 print str(relation.revision) + "\t" + str(relation.author).decode("utf-8") + "\t" + str(target) + "\t" + str(self.revisions[target].contributor_name).decode("utf-8") + "\t" + str(relation.deleted.get(target, [])) + "\t" + str(relation.undo_reintro.get(target, [])) + "\t" + str(relation.undo_delete.get(target, [])) + "\t" + str(relation.reintroduced.get(target, [])) + "\t" + str(relation.redeleted.get(target, []))  + "\t" + str(relation.added)

            #print str(relation.revision) + "\t" + (relation.author).decode("utf-8")   + "\t" + str(relation.deleted) + "\t" + str(relation.undo_reintro) + "\t" + str(relation.undo_delete) + "\t" + str(relation.reintroduced)  + "\t" + str(relation.redeleted) + "\t" + str(relation.added)  + "\t"  +  str(relation.total_tokens) + "\t" + str(relation.self_deleted) + "\t" + str(relation.self_undo_reintro) + "\t" + str(relation.self_undo_delete) + "\t" + str(relation.self_reintroduced) + "\t" + str(relation.self_redeleted)
    ...
    def printRevisionJSON(self, revision):
...
if __name__ == '__main__':
    ...
    time1 = time()
    wikiwho = Wikiwho()
    (revisions, order, relations) = wikiwho.analyseArticle(file_name)
    time2 = time()
    ...
    if (output == 'r'):
        ...
        wikiwho.printDyads(relations, order)
    ...
    if (output == 'a'):
        ...
        if (revision == 'all'):
            ...
            wikiwho.printRevisionJSON(revisions[int(revision)])
